...
export function FilteredSectionIndicator({ table }: FilteredSectionIndicator) {
  ...
  if (selectedSection) {
    filteredSectionFooter = `${getSectionCode(selectedSection)}: ${selectedSection.level} ${selectedSection.type} ${
      selectedSection.name
        ? `- ${selectedSection.name}`
        : getSectionCode(selectedSection).split('.').length > 2
          ? '- General'
          : ''
    }`;
  }
